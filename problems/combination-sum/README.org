...
func backtrack(combination []int, presum int, target int, candidates []int, pos int, res *[][]int) {
    if pos == len(candidates) {
        return
    }
    v := candidates[pos]
    for i:=0; presum+i*v<=target; i++ {
        l := make([]int, i)
        for j:=0; j<i; j++ {
            l[j] = v
        }
		...
		combination2 = append(combination2, l...)
        if presum+i*v == target {
            *res = append(*res, combination2)
        } else {
            backtrack(combination2, presum+i*v, target, candidates, pos+1, res)
        }
    }
}
...
func combinationSum(candidates []int, target int) [][]int {
    res := [][]int{}
    backtrack([]int{}, 0, target, candidates, 0, &res)
    return res
}
#+END_SRC
