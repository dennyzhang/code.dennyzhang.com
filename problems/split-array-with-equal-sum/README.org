...
:PROPERTIES:
:type:     hashmap, twosum, splitarray, inspiring
:END:
---------------------------------------------------------------------
...
## Basic Ideas: presum
##
##  Cut the array into 4 parts, while removing i, j, k
##
##  Choose middle point first: Loop each j.
##  Thus instead of 3 loops, we only need 2 loops
##
##  Use hashmap to reduce 3 loops to 2 loops
##
...
class Solution:
    def splitArray(self, nums: List[int]) -> bool:
        ...
        for j in range(3, n-2):
            s = set()
            for i in range(j-1):
                if presums[i] == (presums[j]-presums[i+1]):
                    s.add(presums[i])
            for k in range(j+2, n-1):
                if (presums[k]-presums[j+1]) == (presums[n]-presums[k+1]) \
                    and (presums[k]-presums[j+1]) in s:
                    return True
        Return False
#+END_SRC
