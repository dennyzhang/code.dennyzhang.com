...
:PROPERTIES:
:type:     manydetails, constructstring, string
:END:
---------------------------------------------------------------------
...
- Tag: [[https://code.dennyzhang.com/review-manydetails][#manydetails]], [[https://code.dennyzhang.com/followup-constructstring][#constructstring]], [[https://code.dennyzhang.com/review-string][#string]]
---------------------------------------------------------------------
Given a list of non negative integers, arrange them such that they form the largest number.
...
func largestNumber(nums []int) string {
    numsList := make([]string, len(nums))
    for i, n := range nums {
        numsList[i] = strconv.Itoa(n)
    }

    sort.Slice(numsList, func(i, j int) bool {
        return numsList[i]+numsList[j] > numsList[j]+numsList[i]
    })

    res := strings.Join(numsList, "")
    // remove leading 0s: [0, 0]
